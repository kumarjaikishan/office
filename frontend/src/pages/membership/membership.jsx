                ...
                <div className="grid md:grid-cols-3 gap-8 items-stretch">
                    {pricingPlans.map((plan, i) => (
                        <Reveal key={i} className={`delay-${i * 150}`}>
                            <div className={`relative h-full p-10 rounded-[2.5rem] border transition-all duration-500 group flex flex-col ${
                                plan.isPopular 
                                ...
                                <div className="mb-8">
                                    <div className={`w-12 h-12 rounded-2xl mb-6 flex items-center justify-center border ${
                                        plan.isPopular ? 'bg-orange-500/10 border-orange-500/50 text-orange-500' : 'bg-neutral-800 border-neutral-700 text-gray-400'
                                    }`}>
                                        {plan.icon}
                                    </div>
                                    ...
                                    {plan.features.map((feature, idx) => (
                                        <li key={idx} className="flex items-start gap-4 text-sm">
                                            <div className={`mt-0.5 rounded-full p-0.5 border ${
                                                plan.isPopular ? 'border-orange-500/50 text-orange-500' : 'border-neutral-700 text-neutral-600'
                                            }`}>
                                ...
                                <button className={`w-full py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all active:scale-[0.98] ${
                                    plan.isPopular
                                    ...
                                    {plan.price === "Custom" ? "Contact Solutions" : "Request Demo"}
                                </button>
                            </div>
                        </Reveal>
                    ))}
