...
const BulkMark = ({
  ...
  const attendanceMap = useMemo(() => {
    const map = new Map();
    alreadyAttendance.forEach(a => map.set(a.employeeId._id, a));
    return map;
  }, [alreadyAttendance]);

  // 🔥 Initialize rows ONLY when modal opens + ONLY filtered employees
  useEffect(() => {
    if (!openmodal || !filteredEmployee.length) return;
  ...
  }, [openmodal, filteredEmployee, attendanceMap]);
  ...
  useEffect(() => {
    ...
    if (toall.punchIn || toall.punchOut || toall.status) {
      setcheckedemployee(filteredEmployee.map(e => e._id));
    }
  }, [toall, filteredEmployee, openmodal]);
  ...
  const handleAllSelect = useCallback((e) => {
    setcheckedemployee(e.target.checked ? filteredEmployee.map(e => e._id) : []);
  }, [filteredEmployee]);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    const selectedData = checkedemployee.map(employeeId => {
      ...
      if (record.punchIn) {
        data.punchIn = new Date(`${attandenceDate.format('YYYY-MM-DD')}T${record.punchIn}`).toISOString();
      }
      if (record.punchOut) {
        data.punchOut = new Date(`${attandenceDate.format('YYYY-MM-DD')}T${record.punchOut}`).toISOString();
      }
    ...
    try {
      setisUpdate(true);
      const res = await fetch(`${import.meta.env.VITE_API_ADDRESS}bulkMarkAttendance`, {
        method: 'POST',
        headers: {
  ...
  }, [checkedemployee, rowData, employee, attandenceDate, dispatch]);
