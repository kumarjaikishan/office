...
const Sidebar = () => {
  ...
  const sidebarOpen = Boolean(user?.sidebar); // ✅ always boolean
  ...
  return (
    <div className="w-full h-full px-1 md:px-2">
      ...
      <div className="h-[60px] flex items-center gap-4">
        ...
        <span className={sidebarOpen ? "block capitalize" : "hidden"}>
              ...
              item.isLogout ? (
                <button
                  ...
                  className={`flex w-full mb-1 items-center rounded text-gray-600 hover:bg-teal-50
                       ${sidebarOpen ? "justify-start gap-3 px-2 py-2" : "justify-center h-10"}
                     `}
                >
                  <span className="text-[18px]">{item.icon}</span>
                  {sidebarOpen && <span>{item.menu}</span>}
                </button>
              ) : (
                <NavLink
                  to={item.link}
